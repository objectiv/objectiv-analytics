...
class Map:
    ...
    def retention_matrix(self,
        ...
        # applying end_date filter, we can apply start_date filter only
        # after calculating the cohort distances
        if end_date is not None:
            data = data[data['timestamp'] < datetime.strptime(end_date, '%Y-%m-%d')]
        ...
        if time_format == '%Y':
            ...
            data['cohort_distance'] = data['cohort_distance'].dt.days.astype(dtype=int)

            # if time_period == 'weekly':
            #     # since we don't have math.ceil() like function we'll implement
            #     # it for now in this way `int(n / 7) + (n % 7 > 0)`
            #     n_days = 7
            #
            #     # ratio
            #     data['cohort_distance_ratio'] = (data['cohort_distance'] / n_days).astype(dtype=int)
            #
            #     # remainder
            #     data['cohort_distance_remainder'] = (data['cohort_distance'] % n_days).astype(dtype=int)
            #     data.loc[data.cohort_distance_remainder > 0, 'cohort_distance_remainder'] = 1
            #
            #     # sum of ratio and remainder
            #     data['cohort_distance'] = data['cohort_distance_ratio'] + data['cohort_distance_remainder']
            #     data = data.drop_duplicates(['user_id', 'cohort_distance'], keep='first')

        # applying start_date filter
        if start_date is not None:
            if time_format == '%Y':
                _filter = data['first_cohort'] >= datetime.strptime(start_date,
                                                                    '%Y-%m-%d').year
            elif time_format == '%Y%m':
                date_filter = datetime.strptime(start_date, "%Y-%m-%d").year * 100 \
                              + datetime.strptime(start_date, "%Y-%m-%d").month
                _filter = data['first_cohort'] >= date_filter
            else:
                _filter = data['first_cohort'] >= datetime.strptime(start_date,
                                                                    '%Y-%m-%d')
            data = data[_filter]
        ...
        if time_format == '%Y':
            ...
            retention_matrix['first_cohort'] = retention_matrix['first_cohort'].astype(dtype=str)
            retention_matrix['first_cohort'] = retention_matrix['first_cohort'].str[:4] + \
                                               '-' + retention_matrix['first_cohort'].str[4:]
        else:
            retention_matrix['first_cohort'] = retention_matrix['first_cohort'].dt.\
                strftime('%Y-%m-%d').astype(dtype=str)

        retention_matrix = retention_matrix.set_index('first_cohort')
